<?php
/* -----------------------------------------------------------
   admin/tambah_berita.php  –  Form tambah berita (dipulihkan)
   ----------------------------------------------------------- */
session_start();
...
if ($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $check->bind_param('s',$slug);$check->execute();$check->bind_result($cnt);$check->fetch();$check->close();
  if($cnt>0) $slug .= '-'.time();
  ...
  if(!$msg && $judul && $isi && $katID){
     ...
     $stmt->bind_param('ssssii',$judul,$slug,$isi,$gambar,$katID,$penulis);
     if($stmt->execute()){
        $msg='Berita berhasil disimpan!';
        $judul=$isi='';$katID=0;$slug='';
     }else $msg='Gagal menambahkan berita!';
     $stmt->close();
 ...
 document.addEventListener('DOMContentLoaded',()=>{
   ...
   fileInp.addEventListener('change',()=>{
     if(fileInp.files[0]){
       ...
       if(f.size > <?= $MAX_UPLOAD ?>){alert('Gambar > 5 MB');fileInp.value='';prev.style.display='none';note.textContent='';return;}
...
.form-group{margin-bottom:20px}label{font-weight:600;margin-bottom:8px;display:block;font-size:15px}
input,select,textarea{width:100%;padding:12px;font-size:15px;border:1px solid #ccc;border-radius:4px}
